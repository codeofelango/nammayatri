...
tfAssignedReqToOrder Common.DRideAssignedReq {..} mbFarePolicy becknConfig = do
  ...
  fulfillment <- Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) image tagGroups Nothing isDriverBirthDay isFreeRide (Just $ show EventEnum.RIDE_ASSIGNED) isValueAddNP riderPhone isAlreadyFav favCount
  pure
    Spec.Order
...
tfStartReqToOrder Common.DRideStartedReq {..} mbFarePolicy becknConfig = do
  let Common.BookingDetails {..} = bookingDetails
      ...
      farePolicy = FarePolicyD.fullFarePolicyToFarePolicy <$> mbFarePolicy
      items = UtilsOU.tfItems ride booking merchant.shortId.getShortId Nothing farePolicy booking.paymentId
  fulfillment <- Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing (arrivalTimeTagGroup <> odometerTag) personTag False False (Just $ show EventEnum.RIDE_STARTED) isValueAddNP riderPhone False 0
  pure
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderStatus = Just $ show EventEnum.ACTIVE,
...
tfCompleteReqToOrder Common.DRideCompletedReq {..} mbFarePolicy becknConfig = do
  ...
  fulfillment <- Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing (arrivalTimeTagGroup <> distanceTagGroup <> tollConfidence) personTag False False (Just $ show EventEnum.RIDE_ENDED) isValueAddNP riderPhone False 0
...
tfCancelReqToOrder Common.DBookingCancelledReq {..} becknConfig = do
  ...
  fulfillment <- forM bookingDetails $ \bookingDetails' -> do
    ...
    Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) image arrivalTimeTagGroup Nothing False False (Just $ show EventEnum.RIDE_CANCELLED) isValueAddNP riderPhone False 0
...
tfArrivedReqToOrder Common.DDriverArrivedReq {..} mbFarePolicy becknConfig = do
  let BookingDetails {..} = bookingDetails
      ...
      farePolicy = FarePolicyD.fullFarePolicyToFarePolicy <$> mbFarePolicy
      items = UtilsOU.tfItems ride booking merchant.shortId.getShortId Nothing farePolicy booking.paymentId
  fulfillment <- Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing driverArrivedInfoTags Nothing False False (Just $ show EventEnum.RIDE_ARRIVED_PICKUP) isValueAddNP riderPhone False 0
  pure $
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderFulfillments = Just [fulfillment],
