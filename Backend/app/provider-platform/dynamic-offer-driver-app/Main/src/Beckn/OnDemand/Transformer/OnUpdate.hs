...
buildOnUpdateReqOrderV2 req' mbFarePolicy becknConfig = case req' of
  ...
  OU.EstimateRepetitionBuildReq OU.DEstimateRepetitionReq {..} -> do
    ...
    fulfillment <- Utils.mkFulfillmentV2 Nothing Nothing ride booking Nothing Nothing previousCancellationReasonsTags Nothing False False (Just $ show Event.ESTIMATE_REPETITION) isValueAddNP Nothing False 0 -- TODO::Beckn, decide on fulfillment.state.descriptor.code mapping according to spec-v2
    pure $
      Spec.Order
  ...
  OU.NewMessageBuildReq OU.DNewMessageReq {..} -> do
    ...
    fulfillment <- Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing newMessageTags Nothing False False (Just $ show Event.NEW_MESSAGE) isValueAddNP Nothing False 0 -- TODO::Beckn, decide on fulfillment.state.descriptor.code mapping according to spec-v2
    pure $
      Spec.Order
  ...
  OU.SafetyAlertBuildReq OU.DSafetyAlertReq {..} -> do
    ...
    fulfillment <- Utils.mkFulfillmentV2 Nothing Nothing ride booking Nothing Nothing safetyAlertTags Nothing False False (Just $ show Event.SAFETY_ALERT) isValueAddNP Nothing False 0 -- TODO::Beckn, decide on fulfillment.state.descriptor.code mapping according to spec-v2
    pure $
      Spec.Order
        { orderId = Just ride.bookingId.getId,
          orderFulfillments = Just [fulfillment],
  ...
  OU.StopArrivedBuildReq OU.DStopArrivedBuildReq {..} -> do
    let BookingDetails {..} = bookingDetails
    fulfillment <- Utils.mkFulfillmentV2 Nothing Nothing ride booking Nothing Nothing Nothing Nothing False False (Just $ show Event.STOP_ARRIVED) isValueAddNP Nothing False 0
    pure $
      Spec.Order
        { orderId = Just ride.bookingId.getId,
          orderFulfillments = Just [fulfillment],
  ...
  OU.EditDestinationUpdate OU.DEditDestinationUpdateReq {..} -> do
    ...
    fulfillment <- case updateType of
      OU.SOFT_UPDATE -> do
        ...
        Utils.mkFulfillmentV2SoftUpdate (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing updateDetailsTagGroup personTag False False Nothing isValueAddNP newDestination' False 0
      OU.CONFIRM_UPDATE -> Utils.mkFulfillmentV2 (Just driver) (Just driverStats) ride booking (Just vehicle) Nothing Nothing Nothing False False Nothing isValueAddNP Nothing False 0
    pure $
      Spec.Order
        { orderId = Just $ booking.id.getId,
          orderStatus,
  ...
  OU.QuoteRepetitionBuildReq OU.DQuoteRepetitionReq {..} -> do
    let BookingDetails {..} = bookingDetails
    let previousCancellationReasonsTags = UtilsOU.mkPreviousCancellationReasonsTags cancellationSource
    fulfillment <- Utils.mkFulfillmentV2 Nothing Nothing ride booking Nothing Nothing previousCancellationReasonsTags Nothing False False (Just $ show Event.QUOTE_REPETITION) isValueAddNP Nothing False 0
    pure $
      Spec.Order
        { orderId = Just booking.id.getId,
          orderFulfillments = Just [fulfillment],
  ...
  OU.TollCrossedBuildReq OU.DTollCrossedBuildReq {..} -> do
    let BookingDetails {..} = bookingDetails
    fulfillment <- Utils.mkFulfillmentV2 Nothing Nothing ride booking Nothing Nothing Nothing Nothing False False (Just $ show Event.TOLL_CROSSED) isValueAddNP Nothing False 0
    pure $
      Spec.Order
        { orderId = Just ride.bookingId.getId,
          orderFulfillments = Just [fulfillment],
