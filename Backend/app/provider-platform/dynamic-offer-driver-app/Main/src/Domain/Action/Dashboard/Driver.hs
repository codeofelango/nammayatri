...
driverDocumentsInfo merchantShortId opCity = do
  ...
  transporterConfig <- SCT.findByMerchantOpCityId merchantOpCity.id Nothing >>= fromMaybeM (TransporterConfigNotFound merchantOpCity.id.getId)
...
recordPayment isExempted merchantShortId opCity reqDriverId requestorId serviceName = do
  ...
  transporterConfig <- SCT.findByMerchantOpCityId merchantOpCityId (Just driverId.getId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
...
toggleDriverSubscriptionByService (driverId, mId, mOpCityId) serviceName mbPlanToAssign toToggle vehicleNo = do
  ...
  transporterConfig <- SCT.findByMerchantOpCityId mOpCityId Nothing >>= fromMaybeM (TransporterConfigNotFound mOpCityId.getId)
  if toToggle
    then do
