...
setActivity (personId, _merchantId, merchantOpCityId) isActive mode = do
  ...
  when (isActive || (isJust mode && (mode == Just DriverInfo.SILENT || mode == Just DriverInfo.ONLINE))) $ do
    ...
    transporterConfig <- CQTC.findByMerchantOpCityId merchantOpCityId (Just personId.getId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
...
activateGoHomeFeature (driverId, _merchantId, merchantOpCityId) driverHomeLocationId driverLocation = do
  goHomeConfig <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just driverId.getId)
...
deactivateGoHomeFeature (personId, _, merchantOpCityId) = do
  goHomeConfig <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just personId.getId)
  unless (goHomeConfig.enableGoHome) $ throwError GoHomeFeaturePermanentlyDisabled
  let driverId = cast personId
  driverInfo <- QDriverInformation.findById driverId >>= fromMaybeM DriverInfoNotFound
  unless driverInfo.enabled $ throwError DriverAccountDisabled
...
addHomeLocation (driverId, merchantId, merchantOpCityId) req = do
  cfg <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just driverId.getId)
...
buildDriverEntityRes (person, driverInfo) = do
  transporterConfig <- CQTC.findByMerchantOpCityId person.merchantOperatingCityId (Just person.id.getId) >>= fromMaybeM (TransporterConfigNotFound person.merchantOperatingCityId.getId)
...
makeDriverInformationRes merchantOpCityId DriverEntityRes {..} org referralCode driverStats dghInfo currentDues manualDues md5DigestHash = do
  ...
  CQGHC.findByMerchantOpCityId merchantOpCityId (Just id.getId) >>= \cfg ->
    return $
      DriverInformationRes
...
getNearbySearchRequests (driverId, _, merchantOpCityId) = do
  ...
  transporterConfig <- CQTC.findByMerchantOpCityId merchantOpCityId (Just driverId.getId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
...
respondQuote (driverId, merchantId, merchantOpCityId) req = do
  ...
  where
    ...
    getQuoteLimit dist vehicleVariant tripCategory txnId = do
      driverPoolCfg <- DP.getDriverPoolConfig merchantOpCityId vehicleVariant tripCategory dist (Just txnId)
...
getDriverPaymentsHistoryV2 (driverId, _, merchantOpCityId) mPaymentMode mbLimit mbOffset serviceName = do
  ...
  transporterConfig <- CQTC.findByMerchantOpCityId merchantOpCityId (Just driverId.getId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId) -- check if there is error type already for this
