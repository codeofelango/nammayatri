...
processPayment _ driver orderId sendNotification (serviceName, subsConfig) invoices = do
  transporterConfig <- SCT.findByMerchantOpCityId driver.merchantOperatingCityId (Just driver.id.getId) >>= fromMaybeM (TransporterConfigNotFound driver.merchantOperatingCityId.getId)
