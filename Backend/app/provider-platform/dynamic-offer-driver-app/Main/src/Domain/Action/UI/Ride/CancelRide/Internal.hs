  ...
  fork "cancelRide - Notify BAP" $ do
    case booking.tripCategory of
      DTC.OneWay DTC.OneWayOnDemandDynamicOffer -> do
        ...
        transpConf <- QTC.findByMerchantOpCityId searchReq.merchantOperatingCityId (Just booking.transactionId) >>= fromMaybeM (TransporterConfigNotFound searchReq.merchantOperatingCityId.getId)
