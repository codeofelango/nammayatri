    ...
    makeDriverPoolWithActualDistResult (driverPoolRes, _, ghrId, driverGoHomePoolWithActualDistance) = do
      DriverPoolWithActualDistResult
        { driverPoolResult = makeDriverPoolResult driverPoolRes,
          ...
          intelligentScores = IntelligentScores Nothing Nothing Nothing Nothing Nothing Nothing Nothing transporterConfig.defaultPopupDelay,
    ...
    (a : pprox) -> do
      filtDriverPoolWithActualDist' <-
        case poolType of
          ...
          _ -> do
            driverPoolWithActualDist <- computeActualDistance driverPoolCfg.distanceUnit merchantId merchantOperatingCityId pickup (a :| pprox)
    ...
    mkSpecialZoneQueueActualDistanceResult dpr = do
      DriverPoolWithActualDistResult
        { driverPoolResult = dpr,
          ...
          intelligentScores = IntelligentScores Nothing Nothing Nothing Nothing Nothing Nothing Nothing 0,
          isPartOfIntelligentPool = False,
          pickupZone = False,
          specialZoneExtraTip = Nothing,
          keepHiddenForSeconds = Seconds 0,
...
calculateDriverCurrentlyOnRideWithActualDist calculateReq@CalculateDriverPoolReq {..} poolType batchNum currentSearchInfo = do
  ...
  case driverPool of
    ...
    (a : pprox) -> do
      ...
      driverPoolWithActualDistFromDestinationLocation <- computeActualDistance driverPoolCfg.distanceUnit merchantId merchantOperatingCityId pickup driverPoolResultsWithDriverLocationAsDestinationLocation
  ...
  where
    ...
    calculateActualDistanceCurrently _driverToDestinationDistanceThreshold DriverPoolResultCurrentlyOnRide {..} = do
      ...
      computeActualDistanceOneToOne driverPoolCfg.distanceUnit merchantId merchantOperatingCityId (LatLong previousRideDropLat previousRideDropLon) temp
    ...
    mkDriverPoolWithActualDistResult defaultPopupDelay distDur = do
      DriverPoolWithActualDistResult
        { driverPoolResult = distDur.origin,
          ...
          intelligentScores = IntelligentScores Nothing Nothing Nothing Nothing Nothing Nothing Nothing defaultPopupDelay,
          isPartOfIntelligentPool = False,
          pickupZone = False,
          specialZoneExtraTip = Nothing,
          keepHiddenForSeconds = Seconds 0,
