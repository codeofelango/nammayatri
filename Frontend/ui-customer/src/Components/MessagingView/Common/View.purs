...
messageNotificationView push state =
  ...
  , visibility $ if ((not state.showNotificationBanner) && state.currentSearchResultType /= CT.QUOTES CT.OneWaySpecialZoneAPIDetails  && state.config.feature.enableChat) && state.config.feature.enableSuggestions 
                  then VISIBLE 
