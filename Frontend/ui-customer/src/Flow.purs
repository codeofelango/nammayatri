...
import Resources.Constants (DecodeAddress(..), decodeAddress, encodeAddress, getKeyByLanguage, getValueByComponent, getWard, ticketPlaceId, dummyPrice, estimateLabelMaxWidth, locateOnMapLabelMaxWidth, markerArrowSize, estimateLabelMaxWidth, locateOnMapLabelMaxWidth, markerArrowSize)
...
homeScreenFlow = do
  ...
  case flow of
    ...
    EDIT_THE_LOCATION state -> do 
      ...
      res <- lift $ lift $ HelpersAPI.callApi $ Remote.makeEditLocationRequest state.data.driverInfoCardState.rideId (Just srcAddress) Nothing
      case res of
        Right _  -> pure unit
        Left err -> do
      ...
      liftFlowBT
        $ runEffectFn1 locateOnMap
            locateOnMapConfig
              ...
              , editPickupLocation = if isLocalStageOn EditPickUpLocation then true else false
              }
      homeScreenFlow
    else
      pure unit
