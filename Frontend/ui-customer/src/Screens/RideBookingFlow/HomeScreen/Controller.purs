...
eval (GetEstimates (GetQuotesRes quotesRes) count ) state = do
  ...
  let 
    ...
    quotes = spy "debug quotes" $ filter filterNonAcAsPerGates $ getSpecialZoneQuotes quotesRes.quotes state.data.config.estimateAndQuoteConfig (state.data.fareProductType == FPT.INTER_CITY)
...
eval (LocationTagBarAC (LocationTagBarV2Controller.TagClicked tag)) state = do 
  case tag of 
    ...
    "AMBULANCE" ->
      let
        updatedState = state { data { fareProductType = FPT.AMBULANCE } }
      in
