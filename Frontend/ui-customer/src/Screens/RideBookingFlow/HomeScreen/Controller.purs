...
import Data.Array ((!!), filter, null, any, snoc, length, head, last, sortBy, union, elem, findIndex, reverse, sortWith, foldl, index, mapWithIndex, find, updateAt, insert, delete, tail, singleton, take)
...
eval (PrimaryButtonActionController (PrimaryButtonController.OnClick)) newState = do
    ...
    case state.props.currentStage of
      ...
      ConfirmingLocation -> do
        ...
        let _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_confirm_pickup"
        let updatedState = state{props{locateOnMap = false}, data { iopState { showMultiProvider = false}}}
        updateAndExit updatedState $  (GoToTripSelectionScreen updatedState) --(UpdatedSource updatedState)
      SettingPrice -> do
                        void $ pure $ performHapticFeedback unit
                        void $ pure $ setValueToLocalStore SELECTED_VARIANT state.data.selectedEstimatesObject.vehicleVariant
                        void $ pure $ cacheRateCard state
...
eval (EstimateChangedPopUpController (PopUpModal.OnButton2Click)) state = do
  let
    updatedState = state { props { isEstimateChanged = false, searchExpire = (getSearchExpiryTime true) } }
  updateAndExit updatedState $ GetQuotes updatedState
...
eval (SearchLocationModelActionController (SearchLocationModelController.SelectorControllerAction (SelectorController.OnClick tripType))) state = do
  ...
  continue state {props { searchLocationModelProps {tripType = tripType,totalRideDuration=state.data.tripEstDuration} }, data {returnTimeUTC = "", startTimeUTC = "",estReturnTimeUTC="" ,tripTypeDataConfig = HomeScreenData.tripTypeDataConfig }}
  else 
    let currentUTC = (getCurrentUTC "")
        ...
        returnTripConfig = calculateReturnInfo year month day hour minute (2*estDuration + 3600 + 19800)
    in
      if state.data.startTimeUTC == "" then continue state  
                                          {data 
      ...
      else continue state {props { searchLocationModelProps {tripType = tripType, totalRideDuration = 2*state.data.tripEstDuration + 3600} }}
...
eval (DateSelectAction title dateResp year month day timeResp hour minute ) state = do 
    if any (_ /= "SELECTED") [dateResp, timeResp] then continue state 
      ...
      let selectedDateString = (show year) <> "-" <> (if (month + 1 < 10) then "0" else "") <> (show (month+1)) <> "-" <> (if day < 10 then "0"  else "") <> (show day)
          ...
          returnTripConfig = calculateReturnInfo year month day hour minute (2*estDuration + 3600 + compareUTCwithEstReturn)
           -- additional check can happen here
          isAfterThirtyMinutes = spy "isAfterThirtyMinutes"  $ ((compareUTCDate selectedUTC (getCurrentUTC "")) > (30 * 60))
          ...
          validRoundTripReturnUTC = spy "validRoundTripReturnUTC" $ (selectedUTC > state.data.estReturnTimeUTC && title == "Return")

          bookingDurationCheck = spy "bookingDurationCheck" $ ((title =="Return") && (compareUTCDate state.data.startTimeUTC selectedUTC) < (48 * 60 * 60))
                           ...
                           tripTypeDataConfig {
                            ...
                            tripReturnData = Just HomeScreenData.dummyTripTypeData { 
                             ...
                             props{
                              ...
                              {totalRideDuration = (2*estDuration + 3600)
                              }
                            }
                            }
                    else if validDate && isAfterThirtyMinutes && title == "Return" && selectedUTC > state.data.estReturnTimeUTC && ((compareUTCDate selectedUTC state.data.startTimeUTC) < (48 * 60 * 60)) then
                           state { data 
  ...
  if hasQuotes && topProviderCheck then do -- if choosing multiple provider is not enabled then only show ny
    ...
    exit $ SelectEstimate newState 
      { data
        ...
        -- , rateCard
        --   { vehicleVariant = defaultQuote.vehicleVariant
