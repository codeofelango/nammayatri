...
eval (GetRideConfirmation (RideBookingRes response)) state = do
  ...
  let currentStage =  
      ...
      newState = state {  props { currentStage = currentStage
                        ...
                        , data { driverInfoCardState = getDriverInfo state.data.specialZoneSelectedVariant (RideBookingRes response) (state.data.fareProductType == ST.ONE_WAY_SPECIAL_ZONE) state.data.driverInfoCardState }
                        }
