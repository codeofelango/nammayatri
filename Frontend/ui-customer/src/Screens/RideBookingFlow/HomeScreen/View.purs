...
import Debug (spy)
import Presto.Core.Types.Language.Flow (getState)
            ...
            case initialState.props.currentStage of
              ...
              RideAccepted -> do
                ...
                if ((getValueToLocalStore DRIVER_ARRIVAL_ACTION) == "TRIGGER_WAITING_ACTION") then do
                  ...
                  when 
                    (initialState.data.currentSearchResultType == CTP.QUOTES CTP.OneWaySpecialZoneAPIDetails) $ do
...
getMapHeight state = V (if state.data.currentSearchResultType == CTP.QUOTES CTP.OneWaySpecialZoneAPIDetails then (((screenHeight unit)/ 4)*3) 
...
--      ]
  ...
  -- where disableSuggestions :: HomeScreenState -> Boolean
        -- disableSuggestions state = (state.data.rideType == RideType.RENTAL_RIDE && state.props.currentStage == RideStarted) || state.data.currentSearchResultType == CTP.QUOTES CTP.OneWaySpecialZoneAPIDetails || not state.data.config.feature.enableChat || not state.data.config.feature.enableSuggestions
...
otpAndWaitView push state =
  ...
  , accessibility DISABLE
    ...
    , otpView push state
  ...
  ] <> if (isQuotes state.data.currentSearchResultType || state.data.driverInfoCardState.driverArrived) then 
        [(PrestoAnim.animationSet [ fadeIn true ] $ 
        linearLayout
        [ width WRAP_CONTENT
        , height WRAP_CONTENT
        ...
        , shadow $ Shadow 0.1 0.1 10.0 24.0 Color.greyBackDarkColor 0.5
          ...
          , height WRAP_CONTENT
          , accessibility ENABLE
          , accessibilityHint $ (if (isQuotes state.data.currentSearchResultType) then "O T P Expiry Info" else "Wait time info : ") <> "Button : Select to learn more about " <> if (isQuotes state.data.currentSearchResultType) then "O T P Expiry Time" else "wait time"   
          , text $ if (isQuotes state.data.currentSearchResultType) then getString EXPIRES_IN else getString WAIT_TIME
          , color Color.black700
          , padding $ Padding 12 0 4 if os == "IOS" then 0 else 3
          ] <> FontStyle.body22 TypoGraphy
        , imageView
...
waitTimeHint state = (if (isQuotes state.data.currentSearchResultType) then "O T P Expires in : " else "Wait Time : ") <> case DS.split (DS.Pattern ":") state.data.driverInfoCardState.waitingTime of
                        [minutes, seconds] -> do 
            ...
            if ((getValueToLocalStore TRACKING_ID) == trackingId) then do
              ...
              if (getValueToLocalStore TRACKING_ENABLED) == "False" then do
                ...
                if (srcLat /= 0.0 && srcLon /= 0.0 && dstLat /= 0.0 && dstLon /= 0.0) then do 
                  _ <- pure $ removeAllPolylines ""
                  void $ liftFlow $ drawRoute (walkCoordinate srcLat srcLon dstLat dstLon) (0) {points : []} "DOT" "#323643" false markers.srcMarker markers.destMarker 8 "DRIVER_LOCATION_UPDATE" "" "" specialLocationTag (getNewIDWithTag "CustomerHomeScreen")
                  else pure unit
                void $ delay $ Milliseconds duration
                driverLocationTracking push action driverArrivedAction updateState duration trackingId state routeState
                pure unit
                ...
                if (srcLat /= 0.0 && srcLon /= 0.0 && dstLat /= 0.0 && dstLon /= 0.0) then do 
                  _ <- pure $ removeAllPolylines ""
                  liftFlow $ drawRoute (fromMaybe {points : []} points) (fromMaybe 0 routeDistance) (fromMaybe {points : []} rentalPoints) "LineString" "#323643" true markers.srcMarker markers.destMarker 8 "DRIVER_LOCATION_UPDATE" "" (metersToKm (fromMaybe 0 routeDistance) (state.props.currentStage == RideStarted)) specialLocationTag (getNewIDWithTag "CustomerHomeScreen")
                  else pure unit
                ...
                case state.data.route of
                  Just (Route route) -> do
                        ...
                        if locationResp.isInPath then do
                          ...
                          liftFlow $ runEffectFn1 updateRoute updateRouteConfig { json = newPoints, destMarker =  markers.destMarker, eta =  (metersToKm locationResp.distance (state.props.currentStage == RideStarted)), srcMarker =  markers.srcMarker, specialLocation = specialLocationTag, zoomLevel = zoomLevel, pureScriptID = (getNewIDWithTag "CustomerHomeScreen") }
...
isQuotes :: CTP.SearchResultType -> Boolean
isQuotes (CTP.QUOTES _) = true
isQuotes _ = false