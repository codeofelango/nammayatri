            ...
            case initialState.props.currentStage of
              SearchLocationModel -> case initialState.props.isSearchLocation of
                LocateOnMap -> do
                  ...
                  void $ runEffectFn2 storeCallBackLocateOnMap (\key lat lon -> push $ UpdateLocation key lat lon) (handleLocateOnMapCallback "HomeScreen")  -- UpdatePickupLocation instead of UpdateLocation
                  pure unit
                _ -> do
              ...
              RideAccepted -> do
                ...
                if ((getValueToLocalStore DRIVER_ARRIVAL_ACTION) == "TRIGGER_WAITING_ACTION") then do
                  ...
                  when 
                    (initialState.data.fareProductType == FPT.ONE_WAY_SPECIAL_ZONE) $ do
            ...
            ([ frameLayout
                ...
                , if not state.data.config.feature.enableSpecialPickup then
                    ...
                    [ textView
                        ...
                        , cornerRadius 5.0
                        , visibility $ boolToInvisibility $ showLabel && ((state.props.currentStage == ConfirmingLocation) || (state.props.currentStage == EditPickUpLocation) || state.props.locateOnMap)
                    ...
                    , imageView
                        ...
                        , visibility $ boolToVisibility ((state.props.currentStage == ConfirmingLocation) || (state.props.currentStage == EditPickUpLocation) ||  state.props.locateOnMap)
                        ]
                    ]
                  else
                    linearLayout
                    ...
                    , accessibility DISABLE
                    , orientation VERTICAL
                    , visibility $ boolToVisibility $ state.data.config.feature.enableSpecialPickup && ((state.props.currentStage == ConfirmingLocation) || (state.props.currentStage == EditPickUpLocation) || state.props.locateOnMap)
                    ][ imageView
                        [ width WRAP_CONTENT
                        , height WRAP_CONTENT
                        , accessibility DISABLE_DESCENDANT
...
driverLocationTracking push action driverArrivedAction updateState duration trackingId state routeState expCounter = do
            ...
            if ((getValueToLocalStore TRACKING_ID) == trackingId) then do
              if (getValueToLocalStore TRACKING_ENABLED) == "False" then do
                ...
                if (srcLat /= 0.0 && srcLon /= 0.0 && dstLat /= 0.0 && dstLon /= 0.0) then do 
                  ...
                  if (isLocalStageOn EditPickUpLocation) then do pure unit else void $ liftFlow $ drawRoute [routeConfig] (getNewIDWithTag "CustomerHomeScreen")
                  else pure unit
                void $ delay $ Milliseconds duration
                driverLocationTracking push action driverArrivedAction updateState duration trackingId state routeState expCounter
                ...
                if (isLocalStageOn EditPickUpLocation) then do pure unit
                  else do
                        ...
                        case ((routeResp) !! 0), ((routeRespAdvanced) !! 0), hasCurrentLocAndPrevDropLoc of
                          Just (Route routes), Just (Route routesAdvanced), true -> do
                            ...
                            let newPointsAdv = points
                                ...
                                driverWithinPickupThreshold = distanceBwDriverAndPickup > state.data.config.mapConfig.locateOnMapConfig.editPickUpRadius
                                callback = if state.data.config.feature.enableEditPickupLocation then runFn2 getMarkerCallback push MarkerLabelOnClick else ""
  ...
  , markerSizeHeight = 30.0}
                                destMarkerConfig' = defaultMarkerConfig{ markerId = markers'.destMarker, pointerIcon = markers'.destMarker, primaryText = state.data.driverInfoCardState.source, anchorV = 1.0, markerCallback = callback, actionImage = getPickupMarkerActionImageConifg state.data.config.feature.enableEditPickupLocation driverWithinPickupThreshold}
                            ...
                            if (isLocalStageOn EditPickUpLocation) then do pure unit else liftFlow $ drawRoute [normalRoute,normalAdvRouteConfig] (getNewIDWithTag "CustomerHomeScreen")
                          ...
                          Just (Route routes), Nothing, false -> do
                            ...
                            if (srcLat /= 0.0 && srcLon /= 0.0 && destLat /= 0.0 && destLon /= 0.0) then do 
                              ...
                              void $ pure $ setValueToLocalStore DRIVER_WITHIN_PICKUP_THRESHOLD $ if driverWithinPickupThreshold then "true" else "false"
                              if (isLocalStageOn EditPickUpLocation) then do pure unit else liftFlow $ drawRoute routeArray (getNewIDWithTag "CustomerHomeScreen")
                            else pure unit
                      ...
                      Just (Route route), _  -> do
                            ...
                            if locationResp.isInPath then do
                              let newPoints = { points : locationResp.points}
                                  ...
                                  mapRouteConfig = if onUsRide then specialLocationTag else specialLocationTag{dashUnit = 30, gapUnit = 20}
                                  callback = if state.data.config.feature.enableEditPickupLocation then runFn2 getMarkerCallback push MarkerLabelOnClick else ""
                                  destMarkerConfig =  defaultMarkerConfig { markerId = markers.destMarker, pointerIcon = if hasCurrentLocAndPrevDropLoc then "dummy_dest" else markers.destMarker, markerCallback = callback, actionImage = getPickupMarkerActionImageConifg state.data.config.feature.enableEditPickupLocation driverWithinPickupThreshold}
...
getPickupMarkerActionImageConifg :: Boolean -> Boolean -> ActionImageConfig 
getPickupMarkerActionImageConifg pickupFeatureEnabled driverWithinPickupThreshold = 
  if ((any (\stage -> isLocalStageOn stage) [ RideAccepted, ChatWithDriver]) && pickupFeatureEnabled && driverWithinPickupThreshold) then 
    defaultActionImageConfig {
      ...
      height = 30,
      width = 30,
      background = if os == "IOS" then "#2194FF" else "ic_blue_bg",
      orientation = "HORIZONTAL",
      padding = if os == "IOS" then {left : 12, top : 12, right : 12, bottom : 12} else {left : 16, top : 16, right : 16, bottom : 16},
      layoutMargin = {left : 15, top : 13, right : 15, bottom : 15},
      layoutPadding = {left : 0, top : 0, right : 0, bottom : 0}
    }
    else defaultActionImageConfig
