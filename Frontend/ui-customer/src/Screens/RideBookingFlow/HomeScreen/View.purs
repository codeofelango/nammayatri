  ...
  frameLayout
    ...
    [  linearLayout
       ...
       [ relativeLayout
            ...
            , if state.props.currentStage == GoToTripSelect then (selectTripView2 push state) else emptyTextView state
...
homeScreenViewV2 push state = let 
    cityConfig = getCityConfig state.data.config.cityConfig (getValueToLocalStore CUSTOMER_LOCATION)
    in
    relativeLayout
...
selectTripView2 :: forall w. (Action -> Effect Unit) -> HomeScreenState -> PrestoDOM (Effect Unit) w
selectTripView2 push state = 
  linearLayout [
    width MATCH_PARENT
  , height MATCH_PARENT
  , orientation VERTICAL
