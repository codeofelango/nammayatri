    ...
    , orientation VERTICAL
        ...
        [ linearLayout
          ...
          , linearLayout
            ...
            , orientation VERTICAL
            , visibility if state.props.userReferralType == DRIVER_REFERRAL then VISIBLE else GONE
        ...
        , linearLayout
          ...
          , padding $ Padding 12 12 12 12
            ...
            , linearLayout
              ...
              , background "#FFFFFF"
              , cornerRadius 12.0
            ](DA.mapWithIndex  (\index item -> referralBar index item push state ((isJust state.props.currentReferralItem) && item.id == (fromMaybe "" state.props.currentReferralItem))) (referralList state) )]
        , referredUsersTextView DRIVER push state
        -- , referredUsersTextView CUSTOMER push state
        , rideLeaderBoard push state
      ]

referralBar :: forall w . Int -> {id :: String , avatar_image :: String, text :: String, qr_code_image :: String } -> (Action -> Effect Unit) -> DriverReferralScreenState -> Boolean -> PrestoDOM (Effect Unit) w
referralBar index item push state isqrvisible= 
  linearLayout
    ...
    , appQRCode item.qr_code_image push state isqrvisible
    , linearLayout
            [ width MATCH_PARENT
            , height WRAP_CONTENT
            , gravity CENTER
            ...
            , margin $ MarginTop 10
              ...
              , margin $ MarginRight 12
              , stroke $ "1," <> Color.grey900
              , onClick push $ const $ ShareOptions item.id
              ][imageView
                [ width $ V 16
                , height $ V 16
    ...
    , gravity CENTER
         ...
         , linearLayout
            ...
            , gravity CENTER
             ...
             , buttonOptions (getString SHARE) "ny_ic_app_share" (ShareOptions "driver_referral") push state
             ]
     ]
...
referredUsersTextView :: forall w . ReferredUserType -> (Action -> Effect Unit) -> DriverReferralScreenState -> PrestoDOM (Effect Unit) w
referredUsersTextView val push state =
    linearLayout
        [ width MATCH_PARENT
        , height WRAP_CONTENT
        , background Color.blue600
    ...
    , onClick push $ const $ BackPressed
      ...
      linearLayout
        ...
        , padding $ Padding 12 12 12 12
          ...
          , appQRCode "ny_driver_app_qr_code" push state true
          , PrimaryButton.view (push <<< PrimaryButtonActionController state) (primaryButtonConfig state)
         ]
    ]

appQRCode :: forall w . String -> (Action -> Effect Unit) -> DriverReferralScreenState -> Boolean -> PrestoDOM (Effect Unit) w
appQRCode image push state isqrvisible = 
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
      , gravity CENTER
  ...
  linearLayout
    ...
    , height WRAP_CONTENT
      ...
      , imageView
          [ height $ V 0
          , width $ V 0
          , id $ getNewIDWithTag "CustomerReferralQRScreen"
          , imageWithFallback $ fetchImage FF_ASSET "ny_customer_app_qr_code"
          ]
      , imageView 
          [ height $ V 0
          , width $ V 0
...
referralList :: DriverReferralScreenState -> Array {id :: String , avatar_image :: String, text :: String, qr_code_image :: String }
referralList state = ([
  {id : "customer_referral" , avatar_image : "ny_ic_customer_avatar", text : getString CUSTOMER_TEXT, qr_code_image : "ny_customer_app_qr_code"}
] <> if state.props.userReferralType == CUSTOMER_AND_DRIVER_REFERRAL then [{id : "driver_referral" , avatar_image : "ny_ic_driver_avatar", text : getString DRIVER_TEXT, qr_code_image : "ny_driver_app_qr_code"}] else [])

referredTextList :: DriverReferralScreenState -> Array {referred_text :: String, info_id :: ReferredUserType, count :: String}
referredTextList state = case state.props.userReferralType of
                            CUSTOMER_AND_DRIVER_REFERRAL -> [{referred_text : getString REFERRED_CUSTOMERS, info_id : CUSTOMER, count : state.data.activatedCustomers},
                                                             {referred_text : getString REFERRED_DRIVERS, info_id : DRIVER, count : state.data.referredDrivers}]
                            DRIVER_REFERRAL -> [{referred_text : getString REFERRED_DRIVERS, info_id : DRIVER, count : state.data.referredDrivers}]
                            CUSTOMER_REFERRAL -> [{referred_text : getString REFERRED_CUSTOMERS, info_id : CUSTOMER, count : state.data.referredCustomers},
