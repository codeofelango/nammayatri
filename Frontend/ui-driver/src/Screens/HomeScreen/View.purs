...
gotoRecenterAndSupport state push =
  ...
  , scrollBarX false
    ...
    , visibility $ boolToVisibility $ spy "boolToVisibility" $ not (DA.any (_ == state.props.currentStage) [RideAccepted, RideStarted, ChatWithCustomer] || not state.props.statusOnline)
    ][ linearLayout
        [ width WRAP_CONTENT
...
driverDetail push state =
  let driverImage = case state.data.gender of
      ...
      pillConfig = driverStatusIndicators state
  in
  linearLayout
  [ width MATCH_PARENT
  , height WRAP_CONTENT
