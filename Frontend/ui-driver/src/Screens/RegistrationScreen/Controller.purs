...
eval (RegistrationAction step ) state = do
       ...
       case item of 
          ...
          PROFILE_PHOTO -> if state.data.cityConfig.enableHvSdk == true then continueWithCmd state [ pure $ CallHV "ny-selfie-flow" ""] else exit $ DocCapture state item
          AADHAAR_CARD -> if state.data.cityConfig.enableHvSdk == true then continueWithCmd state [ pure $ CallWorkFlow "ny-aadhaar-flow"] else exit $ DocCapture state item
          PAN_CARD  -> if state.data.cityConfig.enableHvSdk == true then continueWithCmd state [ pure $ CallWorkFlow "ny-pan-flow"] else exit $ DocCapture state item
